...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│   Display   │    │  Capturer   │    │   Encoder   │    │   Network   │
│   Service   │    │  (Platform) │    │   (Codec)   │    │   (TCP/UDP) │
└──────┬──────┘    └──────┬──────┘    └──────┬──────┘    └──────┬──────┘
       │                  │                  │                  │
       │ 1. 初始化采集器  │                  │                  │
       │─────────────────>│                  │                  │
...
**关键交互点**：
1. **初始化阶段**：采集器和编码器初始化，协商编码格式
2. **采集循环**：持续捕获屏幕帧并进行编码
3. **QoS 控制**：根据网络状况动态调整编码参数
4. **显示器变化**：检测并响应显示器配置变化

### 4.5 接收端解码渲染序列图

```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│   Network   │    │  Video      │    │   Decoder   │    │   Renderer  │
│   (TCP/UDP) │    │   Queue     │    │   (Codec)   │    │  (Platform) │
└──────┬──────┘    └──────┬──────┘    └──────┬──────┘    └──────┬──────┘
       │                  │                  │                  │
       │ 1. 接收数据包     │                  │                  │
       │─────────────────>│                  │                  │
...
1. **数据接收**：网络数据包进入视频队列
2. **队列管理**：实现丢弃策略防止队列溢出
3. **解码处理**：根据编码格式选择解码器并解码
4. **渲染回调**：将解码后的帧传递给渲染器显示
5. **FPS 控制**：根据渲染能力调整解码速度
...
- **Decoding**：正在解码编码帧
- **Retrying**：解码失败，正在重试
- **FormatError**：格式错误，需要切换编码器
- **ConvertToRGB**：将解码后的 YUV 转换为 RGB
- **Rendering**：正在渲染解码后的帧
- **Waiting**：等待下一帧数据
...
**QoS 控制策略**：
1. **延迟收集**：收集网络延迟数据并计算平均值
2. **RTT 校正**：减去 RTT 得到实际网络延迟
3. **网络判断**：根据延迟阈值判断网络状况
4. **参数调整**：根据网络状况调整帧率和质量
5. **周期性调整**：每 3 秒调整一次，避免频繁变化
